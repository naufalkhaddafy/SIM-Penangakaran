...
class ProduksiController extends Controller
{
    ...
    public function CreateProduksiTelur($id)
    {
        ...
        // $cek=Produksi::where('kandang_id'. $id)->latest();
        // $status_telur=$cek->status_telur;
        $lastproduksi=Produksi::where('kandang_id', $id)->latest()->first();
        $kdgproduksi=Produksi::where('status_telur', 'pertama')->where('kandang_id', $id)->latest()->first();

        if(Produksi::where('status_telur', 'pertama')){
        ...
        }else if(Produksi::where('status_telur', 'kedua')){
            $tgl_akan_bertelur=date('Y-m-d', strtotime('+4 days', strtotime($lastproduksi->tgl_bertelur)));
            $tgl_akan_menetas=date('Y-m-d', strtotime('+13 days', strtotime($lastproduksi->tgl_bertelur)));;
            $tambahjadwal = [
                'produksi_id' => $lastproduksi->id,
                'tgl_akan_bertelur' => $tgl_akan_bertelur,
