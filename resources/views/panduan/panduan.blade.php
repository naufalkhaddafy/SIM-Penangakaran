...
@section('title', 'Panduan Perawatan')
                        ...
                        <h3 class="card-title">
                            <td>
                                <button type="button" class="btn btn-block btn-outline-success" onclick="showCreate()">
                                    <ion-icon name="book"></ion-icon> <b>Tambah</b>
                                </button>
                            </td>
                        </h3>
                    </div>
...
@push('js')
    <script>
        ...
        function readData() {
            $.get("{{ url('/show-panduan') }}", function(data) {
                $('#readData').html(data);
            });
        }

        function showRead(id) {
            $.get("{{ url('/modal-read-panduan') }}/" + id, function(data) {
        ...
        function showCreate() {
            $.get("{{ url('/modal-create-panduan') }}", function(data) {
                $('#modalLabel').text('Tambah Panduan')
                $('#showModalBody').html(data);
                $('#showModal').modal('show');
                $('#btnClose').show();
        ...
        function showDelete(id) {
            $.get("{{ url('/modal-delete-panduan') }}/" + id, function(data) {
                $('#modalLabel').text('Delete Data Panduan')
                $('#showModalBody').html(data);
                $('#showModal').modal('show');
                $('#btnClose').show();
                $('#btnSubmit').hide();
